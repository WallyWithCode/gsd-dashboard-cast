...
3. Cancel log task in `__aexit__` cleanup BEFORE process termination (insert BEFORE line 269 `self.process.terminate()`):
   ```python
...
**CRITICAL PLACEMENT:** The log task cancellation MUST be placed BEFORE `self.process.terminate()` (currently line 269). The correct sequence is:

```python
# Line 266: logger.info(f"Stopping FFmpeg process (PID: {self.process.pid})")
# INSERT CANCELLATION HERE (before line 269)
# Line 269: self.process.terminate()
```
