...
<task type="auto">
  ...
  <files>src/video/encoder.py</files>
  <action>
1. Add instance variable to track log task in `__init__` method (around line 65, with other instance vars):
   ```python
   self.log_task = None  # Background task for FFmpeg output logging
   ```

2. Start log forwarding task in `__aenter__` after FFmpeg process starts (after line 211, after "FFmpeg process started" log):
   ```python
   # Start background task to forward FFmpeg output to logs
   self.log_task = asyncio.create_task(self._log_ffmpeg_output())
   ```

3. Cancel log task in `__aexit__` cleanup (after line 266, before process termination):
   ```python
   # Cancel log forwarding task if running
...
- Cancelled before FFmpeg termination to prevent reading from closed stream
- CancelledError caught and suppressed (normal cleanup behavior)

**Why cancel before terminate:**
- FFmpeg.terminate() closes stderr pipe
- If log task tries to read after pipe closed, it gets EOF (breaks loop naturally)
- But cancelling explicitly is cleaner and ensures no race conditions
  </action>
  <verify>
...
- Log task cancelled during cleanup before process termination
- All tests pass
  </done>
</task>
...
3. Check that cleanup happens without exceptions:
   ```bash
...
- [ ] Log task cancelled in __aexit__ before process termination
