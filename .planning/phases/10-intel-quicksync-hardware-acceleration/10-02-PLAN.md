...
Hardware detection module that checks for Intel QuickSync availability via vainfo and ffmpeg -encoders.
  ...
  <how-to-verify>
Test hardware detection in four scenarios (including exception handling):
...
**Scenario 3: Test exception handling - vainfo binary missing**
```bash
# Temporarily rename vainfo to simulate missing binary
docker run --rm dashboard-cast sh -c "mv /usr/bin/vainfo /usr/bin/vainfo.bak 2>/dev/null || true; python3 -c 'from src.video.hardware import HardwareAcceleration; hw = HardwareAcceleration(); result = hw.is_qsv_available(); print(f\"Result: {result}\"); print(\"PASS: FileNotFoundError handled gracefully\" if not result else \"FAIL\")'"
```

Expected:
- Result: False
- Log: WARNING message mentioning "Required command not found"
- PASS: FileNotFoundError handled gracefully

**Scenario 4: Test subprocess timeout handling**
```bash
# Test with mock that simulates timeout (create test script)
...
with mock.patch('subprocess.run', side_effect=subprocess.TimeoutExpired('ffmpeg', 5)):
    hw = HardwareAcceleration()
    result = hw.is_qsv_available()
    print(f"Result: {result}")
    print("PASS: TimeoutExpired handled gracefully" if not result else "FAIL")
EOF
...
Expected:
- Result: False
- Log: WARNING message about "Hardware detection timed out"
- PASS: TimeoutExpired handled gracefully

**Scenario 5: If you have /dev/dri on host** (optional - may not exist in VM without passthrough)
```bash
...
2. "exception handling works" - Detection handles missing vainfo binary gracefully (Scenario 3 passes)
3. "timeout handling works" - Detection handles subprocess timeout gracefully (Scenario 4 passes)
4. Ready to proceed with encoder integration
...
- Detection handles FileNotFoundError when vainfo missing (Scenario 3)
- Detection handles subprocess.TimeoutExpired gracefully (Scenario 4)
- Human verified fallback behavior and exception handling work correctly
</verification>
...
- Detection handles FileNotFoundError when vainfo binary missing
- Detection handles subprocess.TimeoutExpired when subprocess hangs
</success_criteria>
