...
**Base Image FFmpeg Version:**
Current Dockerfile uses `python:3.11-slim` base image. Debian Bookworm (Debian 12, released June 2023) ships FFmpeg 5.1.x by default, which does NOT meet HWAC-07 requirement (FFmpeg 7.0+).

**Solution: Install FFmpeg from Debian testing/trixie**

Add FFmpeg 7.0+ installation BEFORE the apt-get install line:

```dockerfile
# Install FFmpeg 7.0+ from Debian testing (trixie) for OneVPL support
RUN echo "deb http://deb.debian.org/debian/ testing main non-free non-free-firmware" > /etc/apt/sources.list.d/testing.list && \
...
This configuration:
1. Adds Debian testing repository (provides FFmpeg 7.0+)
2. Sets low priority (50) to prevent unintended testing package installations
3. Installs FFmpeg specifically from testing using `-t testing` flag
4. HWAC-07 satisfied: FFmpeg 7.0+ includes OneVPL by default on Debian/Ubuntu

Modify the existing apt-get install line (currently line 5) to include Intel media drivers and VAAPI tools (but NOT ffmpeg, since it's installed above):
...
DO NOT install ffmpeg from Debian stable (provides only FFmpeg 5.1.x).
  </action>
  <verify>
...
- Dockerfile installs FFmpeg 7.0+ from Debian testing repository with verification step
...
<success_criteria>
- Dockerfile adds Debian testing repository with low priority pinning
- Dockerfile installs FFmpeg 7.0+ from testing using -t testing flag (HWAC-07)
- Dockerfile installs Intel media drivers (intel-media-va-driver-non-free, vainfo, libva2, libva-drm2)
- Dockerfile verification step checks FFmpeg 7.0+ and h264_qsv encoder availability
