...
```dockerfile
# Verify FFmpeg version and h264_qsv encoder availability
...
docker run --rm test-qsv sh -c "ffmpeg -version | head -1 && ffmpeg -encoders | grep h264_qsv"
  ...
  <done>Build succeeds, FFmpeg version displayed, h264_qsv encoder appears in ffmpeg -encoders output</done>
</task>
...
- Dockerfile contains intel-media-va-driver-non-free and verification step showing FFmpeg version
...
- Dockerfile verification step checks FFmpeg version and h264_qsv encoder availability
