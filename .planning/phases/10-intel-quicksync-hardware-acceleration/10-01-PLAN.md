...
After apt-get install block, add verification step:
```dockerfile
# Verify h264_qsv encoder is available
RUN ffmpeg -encoders 2>/dev/null | grep h264_qsv || echo "WARNING: h264_qsv not found - hardware acceleration unavailable"
```

Reference: Research RESEARCH.md "Standard Stack" section and "Dockerfile - Install Intel Media Drivers" code example.
...
docker run --rm test-qsv ffmpeg -encoders | grep h264_qsv
  </verify>
  <done>Build succeeds, h264_qsv encoder appears in ffmpeg -encoders output</done>
</task>
...
2. group_add section (requires host group ID lookup):
```yaml
    group_add:
...
DO NOT use hardcoded GIDs like "104" or "44" - use RENDER_GID/VIDEO_GID placeholders.
These will be resolved at runtime or documented for manual configuration.
  </action>
  <verify>
docker-compose config validates successfully
grep -E "(devices|group_add|LIBVA_DRIVER_NAME)" docker-compose.yml shows new configuration
  </verify>
  <done>docker-compose.yml contains devices, group_add, and LIBVA_DRIVER_NAME configuration with explanatory comments</done>
</task>
...
- Dockerfile contains intel-media-va-driver-non-free and verification step
...
- Dockerfile verification step checks for h264_qsv encoder availability
- docker-compose.yml passes /dev/dri device to container
- docker-compose.yml includes group_add for render and video groups
