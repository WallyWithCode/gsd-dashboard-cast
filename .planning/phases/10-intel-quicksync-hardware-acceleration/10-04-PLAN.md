...
must_haves:
  ...
  key_links:
    ...
    - from: docs/PROXMOX_GPU_PASSTHROUGH.md
      ...
      pattern: "vainfo"
    - from: docs/PROXMOX_GPU_PASSTHROUGH.md
      to: scripts/detect-gpu-gids.sh
      via: cross-reference in LXC setup instructions
      pattern: "scripts/detect-gpu-gids\\.sh"
---
...
grep -q "scripts/detect-gpu-gids.sh" docs/PROXMOX_GPU_PASSTHROUGH.md && echo "PASS: Helper script cross-referenced"
  </verify>
  <done>docs/PROXMOX_GPU_PASSTHROUGH.md exists with 150+ lines covering IOMMU setup, LXC container passthrough, VM passthrough, driver installation, troubleshooting, and verification checklist includes scripts/detect-gpu-gids.sh reference</done>
</task>
...
**Note on task structure:** This checkpoint intentionally combines lightweight documentation work (Task 1) with comprehensive phase validation (Task 2). The documentation is a 15-minute task, while verification tests the entire phase end-to-end. This structure is appropriate because:
1. Task 1 is purely documentation (no implementation risk)
2. Task 2 validates all prior plans together (Plans 10-01, 10-02, 10-03)
3. Splitting would create an extra plan with only a verification checkpoint
4. Phase-level integration testing is the right granularity for this validation
  </what-built>
  <how-to-verify>
...
- Guide cross-references scripts/detect-gpu-gids.sh in Step 4 (LXC Container Setup)
...
- Guide cross-references scripts/detect-gpu-gids.sh in LXC Step 4
