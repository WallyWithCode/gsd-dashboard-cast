...
# Get render and video group IDs
getent group render | cut -d: -f3  # Usually 104 or 109
getent group video | cut -d: -f3   # Usually 44
...
**Step 5: (OPTIONAL) Enable GPU passthrough and test QuickSync**

If you want to verify QuickSync hardware acceleration works:

1. Follow docs/PROXMOX_GPU_PASSTHROUGH.md guide (requires Proxmox host access)
2. Configure LXC container or VM with /dev/dri passthrough
3. Update docker-compose.yml group_add with actual render/video GIDs
4. Restart service: `docker-compose up -d`
5. Check health endpoint: `curl http://localhost:8000/health | jq .hardware_acceleration`
   - Expected: `"quicksync_available": true, "encoder": "h264_qsv"`
6. Test cast and verify CPU usage reduction (should see 80-90% reduction vs software)

**Note:** GPU passthrough requires Proxmox configuration and may not be possible in current environment. Software fallback verification (Steps 1-4) confirms graceful degradation works correctly.
  </how-to-verify>
  <resume-signal>
Confirm verification results:

**Required (without GPU passthrough):**
...
**Optional (with GPU passthrough):**
5. "quicksync works" - Health endpoint reports quicksync_available: true
6. "cpu reduced" - CPU usage 80-90% lower with h264_qsv vs libx264
...
- Human verified end-to-end functionality (at minimum software fallback, optionally QuickSync acceleration)
</success_criteria>
