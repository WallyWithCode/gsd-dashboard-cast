...
Update logger.info call in __aenter__ (around line 241-244) to include encoder:
```python
logger.info(
    f"Starting FFmpeg encoder: {encoder} @ {self.quality.resolution[0]}x{self.quality.resolution[1]} "
...
DO NOT change audio codec settings or output format sections.
  ...
  <verify>
python -c "from src.video.encoder import FFmpegEncoder; from src.video.quality import get_quality_config; enc = FFmpegEncoder(get_quality_config('720p')); args = enc.build_ffmpeg_args('/tmp/test.m3u8'); print('h264_qsv' if 'h264_qsv' in args else 'libx264', 'found in args')"
  </verify>
  <done>encoder.py imports HardwareAcceleration, __init__ creates hw_accel instance, build_ffmpeg_args uses encoder from get_encoder_config(), rate control conditional on encoder type, logging includes encoder name</done>
</task>
...
- build_ffmpeg_args() selects encoder based on hardware availability
...
- build_ffmpeg_args() calls get_encoder_config() to determine encoder
- h264_qsv uses ICQ mode (global_quality + look_ahead) from encoder_config
- libx264 uses existing bitrate/preset configuration
- Logger output includes encoder name (h264_qsv or libx264)
